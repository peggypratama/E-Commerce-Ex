...
export default class MyAds extends Component {
    ...
    componentWillMount = () => {
        this.initMyAds().then(async () => {
            await firebase.firestore().collection('Orders').where('MarketeerId', '==', this.state.userID).onSnapshot(async docSnapshot => {
    ...
    componentWillUnmount = () => {
        firebase.firestore().collection('Orders').where('MarketeerID', '==', this.state.userID).onSnapshot(() => { })
    }

    initMyAds = async () => {
        await getCurrentUid().then(async uid => {
            await firebase.firestore().collection('Orders').where('MarketeerID', '==', uid).get().then(async snapshot => {
