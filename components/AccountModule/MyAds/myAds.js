...
export default class MyAds extends Component {
    ...
    componentWillMount = () => {
        this.initMyAds().then(async () => {
            await firebase.firestore().collection('Ads').where('uid', '==', this.state.userID).onSnapshot(async docSnapshot => {
    ...
    componentWillUnmount = () => {
        firebase.firestore().collection('Ads').where('uid', '==', this.state.userID).onSnapshot(() => { })
    }

    initMyAds = async () => {
        await getCurrentUid().then(async uid => {
            await firebase.firestore().collection('Ads').where('uid', '==', uid).get().then(async snapshot => {
    ...
    gotoAd = (adObj, updateCard, pageType) => {
        let temp = {
            'obj': adObj,
