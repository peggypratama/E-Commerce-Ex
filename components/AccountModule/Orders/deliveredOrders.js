...
export default class MyAds extends Component {
    ...
    componentWillMount = () => {
        this.initMyAds().then(async () => {
            await firebase.firestore().collection('Delivery').where('CostUid', '==', this.state.userID).onSnapshot(async docSnapshot => {
    ...
    componentWillUnmount = () => {
        firebase.firestore().collection('Delivery').where('CostUid', '==', this.state.userID).onSnapshot(() => { })
    }

    initMyAds = async () => {
        await getCurrentUid().then(async uid => {
            await firebase.firestore().collection('Delivery').where('CostUid', '==', uid).get().then(async snapshot => {
    ...
    onDeletePress = async item => {
        this.setState({ temp: item })
            ...
            await firebase.firestore().collection('Delivery').doc(item.docID).delete().catch(error => console.log(error.message))
