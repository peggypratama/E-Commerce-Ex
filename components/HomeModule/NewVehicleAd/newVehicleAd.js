...
import { View, ScrollView, KeyboardAvoidingView, Alert, Modal } from 'react-native';
...
export default class NewVehicleAd extends Component {
    ...
    componentWillMount = async () => {
        ...
        if (temp) {
            ...
            this.setState({
                ...
                location: temp['Location'], rentSale: temp['Type'], buttonText: 'EDIT',weight: temp['Weight'],
    ...
    isFormEmpty = () => {
        if (this.state.location && this.state.price != '' && this.state.imagePath && this.state.title != '' && this.state.description != '' && this.state.modelYear != '' && this.state.transmission != '' && this.state.color != '' && this.state.mileage != '')
            return false
    ...
    getAdObj = uid => {
        let adObj = {
            'Title': this.state.title, 'Description': this.state.description, 'uid': uid,
            'Price': this.state.price, 'timestamp': firebase.firestore.FieldValue.serverTimestamp(),
            'outletID': '', 'Category': 'Vehicles', 'Type': 'Rent',
            'Location': this.state.location, 'Deliverable': false,
            ...
            'Model Year': this.state.modelYear, 'Transmission': this.state.transmission,
            'Color': this.state.color, 'Mileage': this.state.mileage,
            'Air Conditioning': this.state.airConditioning, 'Alloy Rims': this.state.alloyRims,
            'Central Locking': this.state.centralLocking, 'Airbags': this.state.airbags,
            'Media Player': this.state.mediaPlayer,
        }
        return adObj
    }
