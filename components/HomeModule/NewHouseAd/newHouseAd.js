...
export default class NewHouseAd extends Component {
    ...
    getAdObj = uid => {
        let adObj = {
            'Title': this.state.title, 'Description': this.state.description, 'uid': uid, 'Type': 'Rent',
            'timestamp': firebase.firestore.FieldValue.serverTimestamp(), 'outletID': '',
            'Category': 'Houses', 'Price': this.state.price, 'Location': this.state.location, 'Deliverable': false,
            ...
            'Completion Year': this.state.completionYear, 'Marhalas': this.state.marhalas, 'Floors': this.state.floors,
            ...
            <View style={{ flex: 1 }}>
                ...
                <KeyboardAvoidingView style={{ flex: 9 }}>
                    <ScrollView>
                        ...
                        <DescriptionSection keyboardType={'phone-pad'} value={this.state.price} margin={responsiveHeight(2)} label={'Price in IDR'} height={responsiveHeight(7)} updateParent={(text) => this.setState({ price: text })} flexSize={8} />
